...
Input
 ...
 Each testcase is a single line with a number n, 0 <= n <= 1 000 000 000. It is the number written on your coin.
...
If you try changing the coin 2 into 3 smaller coins,
 you will get 1, 0 and 0, and later you can get no more than $1 out of them.
...
def exchange(n):
    ...
    if n in cache:
        return cache[n]
    cache[n] = max(n, exchange(n / 2) + exchange(n / 3) + exchange(n / 4))
    return cache[n]
